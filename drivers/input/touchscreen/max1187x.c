...
static u8 compare_Touch_Configuration(struct data *ts) {
	...
	if (ts->max11871_Touch_Configuration_Data[0]) {
		for (i=0; i<RETRY_TIMES; i++) {
			DISABLE_IRQ();
			ret = get_touch_config(ts->client);
			if (ret < 0)
...
static u8 compare_Calibration_Table(struct data *ts) {
	...
	if (ts->max11871_Calibration_Table_Data[0]) {
		for (i=0; i<RETRY_TIMES; i++) {
			DISABLE_IRQ();
			//Get calibration table
			mtpdata[0]=0x0011;
...
static u8 compare_Private_Configuration(struct data *ts) {
	...
	if (ts->max11871_Private_Configuration_Data[0]) {
		for (i=0; i<RETRY_TIMES; i++) {
			DISABLE_IRQ();
			//Get private configuration
			mtpdata[0]=0x0004;
...
static u8 compare_Lookup_Table_X(struct data *ts) {
	...
	if (ts->max11871_Lookup_Table_X_Data[0]) {
		for (i=0; i<RETRY_TIMES; i++) {
			DISABLE_IRQ();
			//Get Lookup table X
			mtpdata[0]=0x0031;
...
static u8 compare_Lookup_Table_Y(struct data *ts) {
	...
	if (ts->max11871_Lookup_Table_Y_Data[0]) {
		for (i=0; i<RETRY_TIMES; i++) {
			DISABLE_IRQ();
			//Get Lookup table Y
			mtpdata[0]=0x0031;
...
static u8 compare_Image_Factor(struct data *ts) {
	...
	if (ts->max11871_Image_Factor_Table[0] && config_num[ts->config_protocol][4]) {
		for (i=0; i<RETRY_TIMES; i++) {
			DISABLE_IRQ();
...
/*			if(PDATA(disp_panel_check) && PDATA(disp_panel)
			...
			if(ts->fw_config->config_touch[0]) {
			...
			if(ts->fw_config->config_private[0]) {
			...
			if(ts->fw_config->config_lin_x[0]) {
			...
			if(ts->fw_config->config_lin_y[0]) {
			...
			if(ts->fw_config->config_ifactor[0] && config_num[ts->config_protocol][4]) {
